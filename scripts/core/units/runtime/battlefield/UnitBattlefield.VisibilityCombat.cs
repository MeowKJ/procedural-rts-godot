...
public sealed partial class UnitBattlefield
{
    ...
    {
        if (!Units.Any(unit =>
            (unit.AttackTargetKind == CombatTargetKind.Building && unit.AttackTargetId is not null)
    ...
    {
        foreach (var unit in Units.Where(unit => unit.AttackTargetKind == CombatTargetKind.Building || unit.MoveMode == MoveCommandMode.Attack))
        {
    ...
    {
        var damagedBuildings = new HashSet<int>();
        var destroyedBuildings = new HashSet<int>();
        ...
        foreach (var simEvent in events)
        {
            if (simEvent is EntityDestroyedEvent destroyed)
            {
                ...
                {
                    destroyedBuildings.Add(destroyedBuildingId);
                }
            ...
            damagedBuildings.Add(targetId.Value);
        }

        var deadIds = damagedBuildings
            .Select(BuildingSnapshot)
            .Where(snapshot => snapshot is { Hp: <= 0 })
            .Select(snapshot => snapshot!.Value.Id)
            .Concat(destroyedBuildings)
            .Distinct()
            .ToList();
        if (deadIds.Count > 0)
        {
