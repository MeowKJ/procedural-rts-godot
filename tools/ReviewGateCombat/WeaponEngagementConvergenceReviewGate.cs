static class WeaponEngagementConvergenceReviewGate
{
    public static void Check(string root, GateResult result)
    {
        ...
        var legacyVisibility = ReviewGateSource.Read(root, "scripts", "core", "units", "runtime", "battlefield", "UnitBattlefield.VisibilityCombat.cs");
        ...
        RequireText(resolution, "public static SpecialAttackHook FireHooks(", "WeaponEngagementResolution must expose generic weapon+ammo special hook routing.", result);
        RequireText(resolution, "weaponDef.Hooks |", "WeaponEngagementResolution must combine weapon hooks with ammo hooks.", result);
        RequireText(ReviewGateSource.Read(root, "scripts", "core", "sim", "SimEvent.cs"), "SpecialAttackHook Hooks", "WeaponFiredEvent must carry generic special attack hook data for presentation/follow-up systems.", result);
        ...
        RequireText(projectile, "WeaponEngagementResolution.FireHooks(context.World, definition)", "Projectile interception must carry generic special attack hook data.", result);
        RequireText(legacyVisibility, "WeaponEngagementResolution.FireHooks(_entityWorld, weapon)", "Legacy UnitBattlefield fire events must carry generic special attack hook data.", result);
