...
void ValidateElementUpgradeModifiers(List<string> failures)
{
    ...
    var otherOwnerEnergy = UpgradeResolver.Damage(world, otherOwner, DamageElementIds.Energy, 1f);
    ...
    RequireCoverage(Nearly(otherOwnerEnergy, 1f), "Element upgrade probe must remain scoped to the owner that completed it.", failures);
