static class HoverTooltipReviewGate
{
    public static void Check(string root, GateResult result)
    {
        ...
        var repairOrderProjection = ReviewGateSource.Read(root, "scripts", "core", "sim", "RepairOrderProjection.cs");
        var inputResources = ReviewGateSource.Read(root, "tools", "CombatBehaviorPresentation", "InputResources.cs");
        ...
        RequireText(preview, "RepairNeedsSupportPreviewLabel()", "Smart repair hover must expose missing repair support when no repairer is selected.", result);
        ...
        RequireText(repairCommand, "RepairCommandStatusText(hoveredUnit.EntityId)", "Accepted unit repair commands must scope stalled funding feedback to the just-targeted repair order.", result);
        RequireText(repairCommand, "RepairCommandStatusText(acceptedTarget)", "Accepted structure repair commands must scope stalled funding feedback to the just-targeted repair order.", result);
        RequireText(repairCommand, "projection.Target == targetEntity", "Repair status must not let older stalled repair orders poison a new accepted repair command.", result);
        RequireText(harvestRepair, "NeedsRepairSupport(PlayerSlotId playerSlotId, UnitInstance target)", "UnitBattlefield must expose missing repair support feedback for damaged friendly units.", result);
        RequireText(harvestRepair, "NeedsRepairSupportBuilding(PlayerSlotId playerSlotId, int buildingId)", "UnitBattlefield must expose missing repair support feedback for damaged friendly structures.", result);
        RequireText(repairOrderProjection, "InsufficientCredits", "Repair order presentation projection must expose insufficient-credit stalls.", result);
        RequireText(repairOrderProjection, "public bool IsStalled", "Repair order presentation projection must expose a compact stalled-state predicate.", result);
        RequireText(harvestRepair, "RepairOrderProjections(PlayerSlotId playerSlotId)", "UnitBattlefield must expose repair order projections for presentation without mutating repair commands.", result);
        RequireText(inputResources, "smart right-click repair projection should expose insufficient-credit repair stalls", "CombatBehavior presentation QA must pin insufficient-credit repair stall feedback.", result);
        ...
        RequireText(englishText, "[\"preview.repair.needSupport\"] = \"NEED REPAIRER\"", "English repair preview must expose missing repair support.", result);
        ...
        RequireText(englishText, "[\"repair.stalled.noCredits\"] = \"Repair stalled: need credits\"", "English repair status must expose insufficient-credit stalls.", result);
        ...
        RequireText(chineseText, "[\"preview.repair.needSupport\"] = \"需要修理单位\"", "Chinese repair preview must expose missing repair support.", result);
        RequireText(chineseText, "[\"preview.repair.invalid\"] = \"无法修理\"", "Chinese repair preview must label invalid repair targets.", result);
        RequireText(chineseText, "[\"repair.stalled.noCredits\"] = \"修理暂停：资金不足\"", "Chinese repair status must expose insufficient-credit stalls.", result);
