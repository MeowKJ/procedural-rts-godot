...
static void AssertHudFactoryExtraction(string root)
{
    ...
    RequireText(hudLayer, "Name = \"CatalogEmptyHint\"", "Right command panel must expose a stable empty-state hint node.");
    ...
    RequireText(hudLayer, "new Vector2(14, 166), new Vector2(270, 72), FontSmall", "Catalog empty-state hints must use a fixed grid-local label that does not resize cards.");
    ...
    RequireText(hudLayer, "RefreshCatalogEmptyHint()", "Right command panel must refresh mode-local empty-state hints when page contents change.");
    RequireText(hudLayer, "CatalogEmptyHintText()", "Right command panel empty-state hint text must be centralized by catalog mode.");
    RequireText(hudLayer, "CatalogModeKind.Build => _visibleBuildCardStates.Count == 0", "Build mode must show an empty-state hint when the selected category has no cards.");
    RequireText(hudLayer, "CatalogModeKind.Train => _visibleCommandCardStates.Count == 0", "Train mode must show an empty-state hint when the selected class has no cards.");
    RequireText(hudLayer, "CatalogModeKind.Upgrades => DefaultUpgradeProjectShellStates.Length == 0", "Upgrades empty-state hint must not overlap rendered project-shell cards.");
    RequireText(hudLayer, "CatalogModeKind.Abilities => _abilityCardStates.Count == 0", "Abilities mode must show a no-source hint before selected units expose abilities.");
    RequireText(englishText, "[\"ui.catalog.empty.build\"]", "English Build empty-state hint text must exist.");
    RequireText(englishText, "[\"ui.catalog.empty.train\"]", "English Train empty-state hint text must exist.");
    RequireText(englishText, "[\"ui.catalog.empty.upgrades\"]", "English Upgrades source-context hint text must exist.");
    RequireText(englishText, "[\"ui.catalog.empty.abilities\"]", "English Abilities empty-state hint text must exist.");
    RequireText(chineseText, "[\"ui.catalog.empty.build\"]", "Chinese Build empty-state hint text must exist.");
    RequireText(chineseText, "[\"ui.catalog.empty.train\"]", "Chinese Train empty-state hint text must exist.");
    RequireText(chineseText, "[\"ui.catalog.empty.upgrades\"]", "Chinese Upgrades source-context hint text must exist.");
    RequireText(chineseText, "[\"ui.catalog.empty.abilities\"]", "Chinese Abilities empty-state hint text must exist.");
