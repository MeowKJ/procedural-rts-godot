...
static void AssertHudFactoryExtraction(string root)
{
    ...
    RequireText(hudLayer, "return GameText.Format(\"ui.catalog.modeSelected\", button.Label, CatalogContextResetText());", "Catalog mode button press must confirm the page and include compact reset context.");
    RequireText(hudLayer, "GameText.Format(\"ui.catalog.modeSelected\", CatalogModeLabelText(next), CatalogContextResetText())", "Keyboard catalog cycling must confirm the page and include compact reset context.");
    RequireText(hudLayer, "private void SetCatalogContextResetText()", "HUD must centralize catalog context reset feedback through the catalog inspector.");
    RequireText(hudLayer, "\"ui.catalog.reset.build\"", "Build context reset feedback must use localized copy.");
    RequireText(hudLayer, "\"ui.catalog.reset.train\"", "Train context reset feedback must use localized copy.");
    RequireText(hudLayer, "CatalogModeKind.Upgrades => GameText.T(\"ui.catalog.reset.upgrades\")", "Upgrades context reset feedback must use localized copy.");
    RequireText(hudLayer, "CatalogModeKind.Abilities => GameText.T(\"ui.catalog.reset.abilities\")", "Abilities context reset feedback must use localized copy.");
    RequireText(hudLayer, "CatalogContextProviderText(CurrentConstructionProviderLaneState(), _selectedConstructionProviderLaneScope)", "Build reset feedback must include the selected construction provider context.");
    RequireText(hudLayer, "CatalogContextProviderText(CurrentProductionProviderLaneState(), _selectedProductionProviderLaneScope)", "Train reset feedback must include the selected production provider context.");
    RequireText(hudLayer, "state.QueueCount,\n            CatalogContextResetText()))", "Provider lane selection must preserve lane confirmation while adding reset context.");
    ...
    RequireText(englishText, "[\"ui.providerLane.selected\"] = \"{0}  {1} providers Q{2}\\n{3}\"", "English provider selected feedback must preserve lane detail plus reset context.");
    RequireText(englishText, "[\"ui.constructionProviderLane.selected\"] = \"{0}  {1} builders Q{2}\\n{3}\"", "English construction provider selected feedback must preserve lane detail plus reset context.");
    RequireText(englishText, "[\"ui.catalog.reset.build\"] = \"RESET Build: {0} / {1}\"", "English Build catalog reset feedback text must be compact.");
    RequireText(englishText, "[\"ui.catalog.reset.train\"] = \"RESET Train: {0} / {1}\"", "English Train catalog reset feedback text must be compact.");
    RequireText(englishText, "[\"ui.catalog.reset.upgrades\"] = \"RESET Upgrades: project context\"", "English Upgrades catalog reset feedback text must be compact.");
    RequireText(englishText, "[\"ui.catalog.reset.abilities\"] = \"RESET Abilities: source context\"", "English Abilities catalog reset feedback text must be compact.");
    ...
    RequireText(chineseText, "[\"ui.providerLane.selected\"] = \"{0}  {1}源 队列{2}\\n{3}\"", "Chinese provider selected feedback must preserve lane detail plus reset context.");
    RequireText(chineseText, "[\"ui.constructionProviderLane.selected\"] = \"{0}  {1}建造源 队列{2}\\n{3}\"", "Chinese construction provider selected feedback must preserve lane detail plus reset context.");
    RequireText(chineseText, "[\"ui.catalog.reset.build\"] = \"重置 建造: {0}/{1}\"", "Chinese Build catalog reset feedback text must be compact.");
    RequireText(chineseText, "[\"ui.catalog.reset.train\"] = \"重置 训练: {0}/{1}\"", "Chinese Train catalog reset feedback text must be compact.");
    RequireText(chineseText, "[\"ui.catalog.reset.upgrades\"] = \"重置 升级: 项目上下文\"", "Chinese Upgrades catalog reset feedback text must be compact.");
    RequireText(chineseText, "[\"ui.catalog.reset.abilities\"] = \"重置 能力: 来源上下文\"", "Chinese Abilities catalog reset feedback text must be compact.");
