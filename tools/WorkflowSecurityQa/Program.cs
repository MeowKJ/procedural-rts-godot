...
static void CheckHostedWindowsReleaseWorkflow(string root, List<string> failures)
{
    ...
    Require(source.Contains("templateVersion -ne $godotVersion", StringComparison.Ordinal), "Windows release must bind the template version to the editor version", failures);
