static class PathfindingAllocationReviewGate
{
    public static void Check(string root, GateResult result)
    {
        ...
        RequireText(pathfindingSystem, "ReferenceEquals(_cachedEnvironment, world.MapEnvironment)", "PathfindingSystem cache keys must preserve immutable MapRuntimeEnvironment identity.", result);
        RequireText(pathfindingSystem, "_cachedEnvironmentDomain == domain", "PathfindingSystem cache keys must include movement domain.", result);
        RequireText(pathfindingSystem, "_obstacles.AddRange(_cachedEnvironmentObstacles);", "PathfindingSystem must copy immutable authored blockers before merging live blockers.", result);
        ...
        RequireText(pathfindingSystem, "domain = spec.Movement.Domain;", "PathfindingSystem must derive pathing domain from UnitSpec movement data.", result); RequireText(pathfindingSystem, "new SharedMoveKey(\n                entity.OwnerId.Value,\n                candidate.Domain", "PathfindingSystem shared moves must group by movement domain.", result); RequireText(pathfindingSystem, "BuildStaticBlockers(world, movingEntityId: 0, group[0].Domain)", "PathfindingSystem shared path blockers must use the grouped movement domain.", result); RequireText(pathfindingSystem, "BuildStaticBlockers(world, entity.Id.Value, domain);", "PathfindingSystem single path blockers must use the entity movement domain.", result);
        ...
        RequireText(verifyAll, "pathfinding-environment-cache-qa", "VerifyAll must run pathfinding environment cache QA.", result);
