static class EnemyProductionAiReviewGate
{
    ...
    private static void RequireEconomyScans(string root, GateResult result)
    {
        ...
        RequireText(queries, "CollectIdleEconomyUnits(", "UnitBattlefield must own idle economy-unit collection.", result); RequireText(queries, "NearestVisibleResourceNode(", "UnitBattlefield must own visible resource-node selection.", result);
