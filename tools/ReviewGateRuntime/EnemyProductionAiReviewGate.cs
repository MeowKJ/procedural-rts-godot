static class EnemyProductionAiReviewGate
{
    ...
    private static void RequireEconomyScans(string root, GateResult result)
    {
        ...
        var productionQueries = ReviewGateSource.Read(root, "scripts", "core", "units", "runtime", "battlefield", "UnitBattlefield.ProductionEconomyQueries.cs");
        ...
        RequireText(economy, "battlefield.CollectIdleEconomyUnits(", "Enemy economy must fill idle harvester storage through the UnitBattlefield query routing.", result); RequireText(economy, "battlefield.NearestVisibleResourceNode(", "Enemy economy must choose resource entities through the UnitBattlefield visibility query routing.", result);
        ...
        RequireText(productionQueries, "CollectIdleEconomyUnits(", "UnitBattlefield must own idle economy-unit collection.", result);
        RequireText(resourceQueries, "NearestVisibleResourceNode(", "UnitBattlefield resource domain must own visible resource-node selection.", result);
        ForbidProductionLinq(economy, "Enemy economy", result);
        ForbidText(economy, "battlefield.Units", "Enemy economy must not scan UnitBattlefield units directly.", result); ForbidText(economy, "battlefield.BuildingSnapshots()", "Enemy economy must not scan UnitBattlefield buildings directly.", result); ForbidText(economy, "battlefield.ResourceNodeProjections()", "Enemy economy must not scan resource nodes directly.", result); ForbidText(economy, "EntityWorld.Visibility", "Enemy economy must not inspect visibility directly.", result); ForbidText(economy, ".Aggregate(", "Enemy base-center calculation must not allocate aggregate delegates.", result);
