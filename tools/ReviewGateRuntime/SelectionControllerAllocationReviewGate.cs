static class SelectionControllerAllocationReviewGate
{
    public static void Check(string root, GateResult result)
    {
        ...
        RequireText(controller, "foreach (var unit in UnitBattlefield!.Units)\n        {\n            if (unit.PlayerSlotId == LocalPlayerSlotId && unit.Selected && IsHarvester(unit))", "SelectionController runtime harvester affordance must scan runtime units directly.", result);
        ...
        RequireText(controller, "List<int> _selectionHotkeyUnitIdBuffer", "SelectionController selection hotkeys must reuse unit-id storage.", result);
        ...
        RequireText(controller, "private int SelectLegacyArmy()", "Legacy select-all-army fallback must use an explicit scan.", result);
        RequireText(controller, "private UnitModel? SelectNextLegacyIdleHarvester()", "Legacy idle-harvester fallback must use an explicit scan.", result);
        ...
        ForbidText(controller, "State.SelectedUnits().Where", "SelectionController selection hotkeys must not allocate selected-unit filters.", result);
