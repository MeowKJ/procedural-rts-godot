...
[ ] AI planners (Economy/Production/Defense/AttackWave/Scout/TacticalMicro) submit
    ...
    Progress: #107 expanded `ObservationView` into a Godot-free read-only player
    snapshot with `ObservedEntity`, `ObservedPlayerState`, and
    `ObservedCommandAffordance` value records. The snapshot covers viewer
    slot/owner/tick, self state, known players, visible entity summaries, and
    command affordances without exposing `GameState`, `UnitBattlefield`, or
    `EntityWorld` mutation objects. Live snapshot building remains separate.
...
[x] ReviewGate system budget cleanup: the old ~16k-line validation system is no
    ...
    evidence drifts from TODO or the review record. Validation tool suites current source budget: 151 C# source files / 19741 total lines across 56 suites; largest C# file tools/CombatBehaviorSkirmish/SkirmishAi.cs has 393 lines; largest suite tools/ReviewGateDomains has 1000 lines. Full `ReviewGate`, historical narrow mode
...
[ ] Per-tick allocation paydown: `SpatialGrid<T>` now exists (M9 debt #2) with an
    ...
    Follow-up: #157 removed remaining pathfinding iterator allocation debt in
    `PathfindingMath.FindSharedCorridor(...)`, `PathfindingMath.ReconstructPath(...)`,
    and legacy `GameState.AssignPath(...)` by replacing `Average` / `Skip` /
    `Select` hot-path usage with explicit scans and index loops; `ReviewGate
    regression` locks these no-iterator contracts. Follow-up: #158 moved
    ...
    contracts in `ReviewGate regression`. Follow-up: #162 reused legacy
    production completion buffers by replacing `Buildings.ToList()` producer tick
    snapshots and `UnitObstacles()` `Select(...).ToList()` spawn-obstacle
    construction with explicit scans and caller-owned storage. Follow-up: #163
    replaced `UnitBattlefield.SelectedCount(...)` `SelectedUnits(...).Count()`
    with an explicit unit scan while preserving the public `SelectedUnits(...)`
    enumerable API. Follow-up: #164 replaced `ConstructionSystem`
    prerequisite completed-building `world.OrderedEntities.Any(...)` with an
    ...
    Follow-up: #97 replaced `AcquireAutoTarget(...)`'s per-unit anonymous
    ...
    Follow-up: #99 reused UnitBattlefield placement validation build-anchor and
    obstacle buffers, converted placement helper queries to explicit caller-owned
    fills, and deleted the unused `SpawnObstacles()` helper. `ReviewGate simhot`
    locks the no-`ToList()` placement-query bridge path. Follow-up: #100 split
    `UnitBattlefieldEnemyAttackWaveAi` into focused partial files before AI loop
    allocation cleanup so the near-400-line runtime AI file does not cross the
    governance warning threshold. `ReviewGate simhot` now locks the partial split
    and keeps each enemy attack-wave AI companion file under 200 lines.
    Follow-up: #101 reused enemy attack-wave AI wave/defense unit buffers and
    command id buffers, replacing wave `ToList()` / reserve `ToHashSet()` /
    ...
    Follow-up: #111 added a reusable target broadphase grid to `TurretCombatSystem`,
    building it once per active-turret tick and querying grid neighbors for automatic
    target selection instead of scanning all entities per turret. Manual target
    priority, range checks, target priority, and tie-break behavior are unchanged;
    `ReviewGate simhot` locks the no-full-scan turret auto-target contract.
    Follow-up: #112 reused a `UnitBattlefield` sim-event drain buffer for
    building-target combat, turret combat, and construction rejection bridge drains,
    replacing `Events.Drain()` snapshot arrays and construction rejection
    `OfType(...).LastOrDefault(...)` with `DrainInto(...)` plus an explicit reverse
    scan. `ReviewGate simhot` locks the no-snapshot bridge drain contract.
    Follow-up: #113 replaced the remaining small runtime bridge `Units.Where(...)`
    filters in `RemoveBuildingTarget(...)` and `ApplySelectionCommandStateToUnits(...)`
    with explicit loops, preserving building-target clearing and selection sync
    semantics. `ReviewGate simhot` locks those no-LINQ bridge filters.
    Follow-up: #117 split BattleRoot minimap sync into a focused partial and reused
    double-buffered HUD minimap unit/building/resource lists, replacing the
    refresh-path `ToList()` / `Select()` materialization. `ReviewGate presentation`
    locks the no-LINQ minimap sync contract.
    Follow-up: #118 reused BattleRoot's HUD alert-line buffer and replaced the
    frame-facing alert `OrderByDescending` / `Take` / `Select` / `ToList`
    materialization with an explicit newest-first four-row fill. `ReviewGate
    presentation` locks the alert HUD buffer contract.
    Follow-up: #119 reused BattleRoot's UnitBattlefield vision-source adapter
    buffer, replacing the frame-facing LINQ `Select` iterator with an explicit
    fill before `GameState.UpdateWorldOnly`. `ReviewGate presentation` locks the
    no-LINQ process bridge contract.
    Follow-up: #123 changed `ProductionSystem` queue enqueue/remove to convert
    producer queue storage to a reusable `List<UnitProductionQueueItem>` once and
    then mutate it in place, removing the copied `new UnitProductionQueueItem[...]`
    array paths while preserving production timing, repeat, rally, refunds, and
    ...
    Follow-up: #135/#136/#137 replaced legacy `GameState` targeting, death
    cleanup, and harvester refinery LINQ/materialization paths with explicit
    best-candidate scans and reusable removal buffers. `ReviewGate simhot` now
    ...
    Follow-up: #141 reused `ControlGroupController` save/recall buffers, replacing
    hotkey save `SelectedUnitIds().ToList()`, empty recall array allocation, legacy
    recall `ToHashSet()`, and double-tap center position list materialization with
    controller-owned group lists plus explicit id/position scans. `ReviewGate
    controlgrouphudallocations` locks the no-allocation save/recall contract.
    Follow-up: #142 split those control group save/recall/id-scan helpers into
    `ControlGroupController.Groups.cs`, leaving both the stable entry file and the
    new group partial at 175 lines. `ControlGroupAllocationReviewGate` now reads
    partial-aware evidence and requires the focused partial.
    Follow-up: #143 reused `ControlGroupController` HUD snapshot buffers, moving
    snapshot generation into `ControlGroupController.Snapshots.cs`, reusing a
    ...
    Follow-up: #147/#148/#149 routed deterministic weapon mount, production queue,
    command queue, and command subject hashing through caller-owned ordering buffers
    plus stable in-place sorts. `EntityStateHash.Ordering.cs` keeps the collection
    hash helpers below file-size warning thresholds, while `ReviewGate regression`
    forbids the old state-hash `OrderBy(...)` / `ThenBy(...)` allocation chains.
    Follow-up: #150/#151/#152 reused legacy `SelectionController` command/preview
    selected-unit scans, legacy `GameState` harvest/rally selected buffers, and
    ...
    contract in `ReviewGate regression`. Follow-up: #176 added a caller-owned
    `PathfindingWorkspace` overload for `PathfindingMath.FindPathWithDebug(...)`,
    routed `PathfindingSystem.PlanPath(...)` through a reusable single-path workspace,
    and split `PathfindingMath.Search.cs` so pathfinding files remain below the
    file-size warning threshold while preserving durable returned path ownership.
    Follow-up: #177 added a caller-owned assignment result buffer overload for
    ...
    path/raw-cell ownership. Follow-up: #179 reused workspace-owned shared-
    corridor assignment point/raw-cell scratch buffers, replacing per-member
    `new List<GridObstacle>(sharedRawCells)` and `new List<PathPoint>()`
    temporary allocations while copying only at the durable assignment result
    boundary. Follow-up: #180 reused dedicated workspace-owned shared-corridor
    passability lookup buffers for assignment LOS checks, replacing local
    `new HashSet<GridObstacle>()` / `new Dictionary<GridObstacle, TerrainLayer>()`
    ...
    iterator allocation. Follow-up: #196 added `UnitMountFacingSource` and routed
    live `UnitInstanceView`, legacy `UnitView`, and `DynamicUnitIcon` unit art draws
    through mount-facing sources instead of dictionary materialization; `ReviewGate
    presentation` locks the no-dictionary draw contract.
